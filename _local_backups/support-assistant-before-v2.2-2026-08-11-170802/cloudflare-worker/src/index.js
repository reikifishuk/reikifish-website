...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/health") return json({ ok: true, service: "ReikiFish Support Assistant", version: "2.1" }, 200, cors);
...
async function analysePrompt(input, env) {
  ...
  const system = `You are the ReikiFish Support Assistant. Interpret a request for support and return only valid JSON with these fields: category, urgent, audience, summary, explanation, nextSteps, followUpQuestion. category must be one of: ${Object.keys(LABELS).join(", ")}. summary is one calm British-English sentence under 35 words. explanation is two practical, compassionate sentences under 80 words total. nextSteps is an array of exactly three short, safe actions. followUpQuestion is one useful question that would help narrow the support. Do not diagnose, decide whether allegations are true, give legal conclusions, promise outcomes, or name organisations. Never tell someone to confront an abuser. If there may be immediate danger, prioritise emergency help.`;
  try {
    ...
    const nextSteps = Array.isArray(parsed.nextSteps)
      ? parsed.nextSteps.map(value => String(value).trim().slice(0, 180)).filter(Boolean).slice(0, 3)
      : fallback.nextSteps;
    return {
      ...
      explanation: String(parsed.explanation || fallback.explanation).slice(0, 600),
      nextSteps: nextSteps.length === 3 ? nextSteps : fallback.nextSteps,
      followUpQuestion: String(parsed.followUpQuestion || fallback.followUpQuestion).slice(0, 240)
    };
