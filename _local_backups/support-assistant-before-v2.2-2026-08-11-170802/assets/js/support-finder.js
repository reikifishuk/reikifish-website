...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderResults(search) {
        ...
        const guidance = search.guidance || {};
        const guidanceSteps = Array.isArray(guidance.nextSteps)
            ? guidance.nextSteps
            : [];
        ...
        resultsContent.innerHTML = `
            ...
            ${(guidance.response || search.assistantMessage)
                ? `
                    <div class="sf-ai-guidance">
                        <span>REIKIFISH SUPPORT ASSISTANT</span>
                        <h3>${escapeHtml(guidance.headline || "A clearer next step")}</h3>
                        <p>${escapeHtml(guidance.response || search.assistantMessage)}</p>
                        ${guidanceSteps.length
                            ? `<ol class="sf-ai-next-steps">${guidanceSteps.map(function (step) {
                                return `<li>${escapeHtml(step)}</li>`;
                            }).join("")}</ol>`
                            : ""
                        }
                        ${guidance.followUpQuestion
                            ? `<div class="sf-ai-follow-up"><strong>To narrow this down:</strong> ${escapeHtml(guidance.followUpQuestion)}</div>`
                            : ""
                        }
                    </div>
                `
