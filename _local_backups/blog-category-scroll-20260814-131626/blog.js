...
/* RF_BLOG_CATEGORY_QUERY_START */
(function () {
  'use strict';

  function requestedCategory() {
    var parameters = new URLSearchParams(window.location.search);
    return String(parameters.get('category') || '').trim();
  }
  ...
  function applyRequestedCategory() {
    ...
    var requestedValue = normalise(requested);

    var controls = Array.prototype.slice.call(
      document.querySelectorAll('[data-category]')
    );

    var matchingControl = controls.find(function (control) {
      var savedCategory = control.getAttribute('data-category');
      var displayedCategory = control.textContent;

      return (
        normalise(savedCategory) === requestedValue ||
        normalise(displayedCategory) === requestedValue
      );
    });
    ...
    if (
      !matchingControl.classList.contains('is-active') &&
      typeof matchingControl.click === 'function'
    ) {
      matchingControl.click();
    }

    matchingControl.scrollIntoView({
      behavior: 'smooth',
      block: 'nearest',
      inline: 'nearest'
    });
  ...
  function initialiseCategoryQuery() {
    if (!requestedCategory()) {
      return;
    }

    if (applyRequestedCategory()) {
      return;
    }
