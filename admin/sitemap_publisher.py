...
# RF_RELATIVE_CANONICAL_FIX_START
def _canonical(text, relative):
    tags = re.findall(r"<link\b[^>]*>", text, re.I)
    values = []
    for tag in tags:
        if re.search(r'\brel=["\x27]([^"\x27]*\s)?canonical(\s[^"\x27]*)?["\x27]', tag, re.I):
            match = re.search(r'\bhref=["\x27]([^"\x27]+)', tag, re.I)
            if match:
    ...
    # Root-relative canonicals are valid and are normalised onto ReikiFish.
    if value.startswith("/") and not value.startswith("//"):
        value = "https://" + DOMAIN + "/" + value.lstrip("/")
    elif value.startswith("//"):
        value = "https:" + value
    elif not re.match(r"^[a-z][a-z0-9+.-]*://", value, re.I):
        # A path-relative canonical is resolved from the website root using
        # the page directory, never from the local filesystem.
        base = relative.parent.as_posix().strip(".")
        joined = "/".join(part for part in (base.strip("/"), value.lstrip("/")) if part)
        value = "https://" + DOMAIN + "/" + joined

    parsed = urlparse(value)
    if parsed.scheme not in ("http", "https") or parsed.netloc.lower() not in (DOMAIN, "www." + DOMAIN):
        raise RuntimeError(f"{relative}: invalid external canonical {values[0]}")
    clean_path = re.sub(r"/{2,}", "/", parsed.path or "/")
    return urlunparse(parsed._replace(scheme="https", netloc=DOMAIN, path=clean_path, query="", fragment="")), "canonical"
# RF_RELATIVE_CANONICAL_FIX_END
